  ...
  <div class="card mt-3">
    ...
    >
      <template #item="slotProps">
        <div
          class="border-1 surface-border border-round m-2 p-3 h-26rem flex flex-column justify-content-between"
        >
          <div class="mb-3">
            <div class="relative mx-auto flex justify-content-center">
              <img
                :src="slotProps.data.image"
                class="w-15rem border-round h-15rem"
                @click="$router.push('/product/' + slotProps.data.id)"
              />
              <Tag
                :value="slotProps.data.rating.rate"
                class="absolute"
